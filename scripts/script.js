...
function getGenres() {
	...
	request.open('GET', 'scripts/genres.json');
	request.send();
...
function buildURL() {
	...
	var url;
	if (selectGenre === 'all') {
		url = 'https://itunes.apple.com/de/rss/' + selectChartType + '/limit=200/explicit=true/xml';
	}
	else {
		url = 'https://itunes.apple.com/de/rss/' + selectChartType + '/limit=200/genre=' + selectGenre + '/explicit=true/xml';
	}
	return url;
}
